...
function YeniSiparisCard({ data }: { data: any }) {
  ...
  if (!data) return null
  return (
    <motion.div className="rounded-2xl overflow-hidden border border-green-200" {...card}>
      {/* Header */}
      <div className="bg-green-500 px-4 py-3 flex items-center gap-3">
        <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white text-base shrink-0">
          ✓
        </div>
        <div className="flex-1">
          <div className="text-white font-semibold text-sm">Sipariş #{data.id} oluşturuldu</div>
          <div className="text-green-100 text-xs mt-0.5">Alınacak siparişlere eklendi</div>
        </div>
      </div>
      ...
      <div className="bg-white divide-y divide-gray-100">
        {data.musteri_ad && (
          <div className="px-4 py-2.5 flex justify-between items-center text-sm">
            <span className="text-gray-500 text-xs">Müşteri</span>
            <span className="font-medium text-gray-800">{data.musteri_ad}</span>
          </div>
        )}
        <div className="px-4 py-2.5 flex justify-between items-center text-sm">
        ...
        {data.tarih && (
          <div className="px-4 py-2.5 flex justify-between items-center text-sm">
            ...
            <span className="text-gray-700 text-xs">{fmtTarihKisa(data.tarih)}</span>
          </div>
        )}
        {data.teslim_tarihi && (
          <div className="px-4 py-2.5 flex justify-between items-center text-sm">
            <span className="text-gray-500 text-xs">Teslim</span>
            <span className="text-gray-700 text-xs">{fmtTarihKisa(data.teslim_tarihi)}</span>
          </div>
        )}
        {data.siparis_notu && (
          <div className="px-4 py-2.5 text-xs text-gray-500 italic">📝 {data.siparis_notu}</div>
        )}
      </div>
      ...
      <div className="bg-gray-50 px-4 py-3 border-t border-gray-100">
        <button
          ...
          className="w-full text-xs font-semibold text-green-700 bg-green-50 border border-green-200
            hover:bg-green-100 active:scale-95 transition-all py-2 rounded-xl"
        >
          Siparişleri Gör →
        </button>
      </div>
